...
public class Node100 extends Node {
    ...
    public void getCurrency(Currency currency, Map<String, Integer> map) {
        if (currency.getAmount() >= 100) {
            ...
            if(a <= bills){
                map.put("100", a);
                Machine.getInstance().getBillCount().put("100", bills - a);
            }else{
                map.put("100", bills);
                Machine.getInstance().getBillCount().put("100", 0);
                b = b + (a - bills)*100;
            }
    ...
    public void refreshPanel(JPanel panel, Map<String, Integer> map) {
        ...
        for (Component component : panel.getComponents()) {
            if(component instanceof PanelSingleBill){
                ...
                if(psb.getLblBill().getText().equals("100")){
                    psb.getTxtBill().setText(map.get("100").toString());
                }
            }
        }
