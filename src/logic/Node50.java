...
public class Node50 extends Node {
    ...
    public void getCurrency(Currency currency, Map<String, Integer> map) {
        if (currency.getAmount() >= 50) {
            ...
            int bills = Machine.getInstance().getBillCount().get("50");
            ...
            if(a <= bills){
                map.put("50", a);
                Machine.getInstance().getBillCount().put("50", bills - a);
            }else{
                map.put("50", bills);
                Machine.getInstance().getBillCount().put("50", 0);
                b = b + (a - bills)*50;
            }
    ...
    public void refreshPanel(JPanel panel, Map<String, Integer> map) {
        ...
        for (Component component : panel.getComponents()) {
            if(component instanceof PanelSingleBill){
                ...
                if(psb.getLblBill().getText().equals("50")){
                    psb.getTxtBill().setText(map.get("50").toString());
                }
            }
        }
    ...
    public int returnAmount(Currency currency) {
        ...
        if (currency.getAmount() >= 50) {
            ...
            if (next != null) {
                return bills*50 + next.returnAmount(new Currency(b));
            }
            return bills*50;
